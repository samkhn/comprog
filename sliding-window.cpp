...
std::string longest_substring_with_atmost_two_distinct_chars(std::string_view s)
{
...
std::string longest_substr_without_repeating_chars(std::string_view s)
{
	std::vector<int> map(128, 0);
	int counter = 0, begin = 0, end = 0, length = 0;
...
int main()
{
	...
	for (int i = 0; i < testCount; ++i) {
		got = longest_substring_with_atmost_two_distinct_chars(
			tests[i].arg);
		printf("%s %s | Got %s, Want %s\n",
			tests[i].name.c_str(),
			(got == tests[i].want ? "PASS" : "FAIL"),
			got.c_str(), tests[i].want.c_str());
	}
	substr_test tests2[] = {
		{"2EmptyTestArg", "", ""},
	...
	for (int i = 0; i < testCount; ++i) {
		got = longest_substr_without_repeating_chars(tests2[i].arg);
		printf("%s %s | Got %s, Want %s\n",
			tests2[i].name.c_str(),
			(got == tests2[i].want ? "PASS" : "FAIL"),
			got.c_str(), tests2[i].want.c_str());
	}
	return 0;
}
