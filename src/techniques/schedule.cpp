...
std::vector<int> Schedule(int job_count, const std::vector<std::pair<int, int>> &ordering) {
	std::vector<int> ts;
	// source will be node(s) with in-degree 0
	std::vector<int> indegrees(job_count, 0);
	std::vector<std::vector<int>> prereqs(job_count, std::vector<int>(job_count, 0));
...
int main() {
	...
	for (const auto &t : tests) {
		...
		if (!Equal(got, t.want)) {
			std::cout << "Failed " << t.name << ". Got: " << ToString(got) << ". Want: " << ToString(t.want) << "\n";
		} else {
