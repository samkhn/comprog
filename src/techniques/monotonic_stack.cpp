...
// Usually good for questions that ask for the next largest/smallest element
// or previous largest/smallest element.
...
//	 that is smaller than the top, the next smaller value from the index at
// the 	 top of the stack is i
...
int main() {
  ...
  for (int v : ng) printf("%d ", v);
  printf("}. Prev greater is\n{ ");
  for (int v : pg) printf("%d ", v);
  printf("}\n");
  return 0;
}
